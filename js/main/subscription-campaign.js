window.addEventListener("load", () => {
    ...
    document.getElementById("subscription-time").innerHTML = days + "d " + hours + "h "
    + minutes + "m " + seconds + "s " + dot + " limited time only";

        // If the count down is finished, write some text
        if (distance < 0) {
            clearInterval(x);
            document.getElementById("subscription-time").innerHTML = "EXPIRED";
        }
    }, 1000);
});
